...
- 规则
  ...
  - **存储自定义类，需要重写hashcode()和equals()方法。**

    <!--存储时需要调用hashcode方法计算存储位置；如果存储位置有元素，则比较的地址，因此需要重写equals方法-->
...
### 2.8 小结（包括HashSet,TreeSet使用场景）

- 如果需要按内容访问集合的元素，则HashSet和TreeSet是合适的
- HashSet的缺点是需要大量的memory
- HashSet计算hashcode值作为索引
- TreeSet存储的元素是有序的，需要实现Comparable接口
- 自定义类存储在HashSet需要重写hashcode()和equals()方法
- Map的使用场景：频繁查询，增加和删除特定元素
- ArrayList的使用场景：频繁遍历，中间删除和增加操作较少
- 选择集合类型需要考虑的内容
  - Whether the collection is *fixed size* or *dynamic*
...
- 规则
  ...
  - 键必须唯一
  - **键类型是自定义类型，则需要重写hashcode()和equals()方法**
  - 添加：put( ),必须提供键
  - 检索：get( ),必须使用一个键
  - 替换：put( )，同时会返回被替换的值
  - 删除：remove( )
